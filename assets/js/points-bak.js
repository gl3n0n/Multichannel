  ...
  self.updateCampaigns = function(brandId)
  {
    ...
    jQuery.ajax({
      ...
      success: function(response) {
        ...
        $(self.channelId).html( jquery("<option/>",{"value":"", text: "--Select a brand--"})).attr("size", 1);

        var curval = $(self.brandId).val();

        if(response.length === 0)
        {
            var promptTextCampaign = ( ! curval) ? "--Select a brand--" : "--Nothing--";
            $(self.campaignId).html( $("<option/>", {value: "", text: promptTextCampaign })).attr("size",1);
            $(self.channelId).html( $("<option/>", {value: "", text: promptTextCampaign })).attr("size",1);
        }
        else
        { 
            $(self.campaignId).attr("size", 5);
            $(self.channelId).html( $("<option/>", {value: "", text: "--Select a campaign--" })).attr("size",1);
        }

        $.each(response, function(idx, val) {
            $(self.campaignId).append( $("<option/>", {
                value: idx,
                text: val
            }));
        });
