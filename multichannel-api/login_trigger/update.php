<?php
	...
	if (empty($trigger_parameter) && empty($start_date) && empty($end_date) && empty($trigger_reward) && 
	    empty($updated_by) && empty($client_id) && empty($brand_id) && empty($channel_id) && empty($campaign_id))
    {
        $response['result_code'] = 405;
        $response['error_txt'] = 'Missing Parameters';
        echo json_encode($response);
    ...
    $response = $trigger->update($client_id, $brand_id, $campaign_id, $channel_id,
							     $trigger_parameter, $start_date, $end_date, $trigger_reward, $updated_by);

    if (!$response)
    {
		$response['result_code'] = 500;
        $response['error_txt'] = 'Error Updating Login Trigger';
    }
