...
class RewardList
{
	...
	public function list_of_redeemable_rewards($pdata=null)
	{
			...
			$query      = "
				SELECT  
					...
					( 
						 ...
						 FROM
							...
							WHERE 
							 ...
							 and
							 custp.SubscriptionId   = cust.SubscriptionId
					      ) >= IFNULL(dtls.Value,0)
					) as redeemable,
					(  SELECT 
					     SUM(IFNULL(custp.Balance,0)) 
					...
					(
					 CURDATE() BETWEEN rlist.DateFrom AND rlist.DateTo
					) dtls_range_ok,
					(
						CURDATE() BETWEEN dtls.StartDate AND dtls.EndDate
					) date_range_ok
				FROM
	...
	public function list_of_rewards_available($pdata=null)
	{
			...
			$query      = "
				SELECT  
					...
					dtls.RewardId,
					dtls.PointsId,
					dtls.Name as RewardDetailsName,
					cust.CustomerId ,
					cust.ClientId   ,
					clnt.CompanyName as ClientName,
					rlist.RewardId  ,
	...
	public function do_redeemed_a_reward($pdata=null)
	{
			...
			SELECT  DISTINCT
				...
				( CURDATE() BETWEEN rlist.DateFrom AND rlist.DateTo ) as rlist_date_range,
			...
			while ($row = $res->fetchRow(MDB2_FETCHMODE_ASSOC))
			{
				...
				*/			
				...
				//413       Insufficient Balance (reward_details.Inventory = 0)
				if ( $row["redeemable"]  <= 0)			   
				{
					$retv['result_code'] = 413;
