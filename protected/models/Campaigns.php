...
class Campaigns extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('ClientId, BrandId, CampaignName, Description, DurationFrom, DurationTo, Type', 'required'),
			array('CreatedBy, UpdatedBy', 'numerical', 'integerOnly'=>true),
			array('ClientId', 'length', 'max'=>11, 'allowEmpty'=>false),
			array('BrandId', 'length', 'max'=>11, 'allowEmpty'=>false),
			array('CampaignName, Description', 'length', 'max'=>100),
			array('Type', 'length', 'max'=>50),
			array('Status', 'length', 'max'=>8),
			array('DateCreated, DateUpdated', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('CampaignId, ClientId, BrandId, CampaignName, Description, DurationFrom, DurationTo, Type, Status, DateCreated, CreatedBy, DateUpdated, UpdatedBy', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		// TO DO: Convert this to relations + scopes
		// SELECT * FROM `campaigns` JOIN `brands` USING (`BrandId`) WHERE `brands`.`BrandId` = 1;
		return array(
			'campaignClients'=>array(self::BELONGS_TO, 'Clients', 'ClientId'),
			'campaignBrands'=>array(self::BELONGS_TO, 'Brands', 'BrandId'),
			'campaignCreateUsers'=>array(self::BELONGS_TO, 'Users', 'CreatedBy'),
			'campaignUpdateUsers'=>array(self::BELONGS_TO, 'Users', 'UpdatedBy'),
		);
	}
